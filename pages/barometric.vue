        ...
        <v-card class="pa-5 pa-sm-7 barometric-hero mb-6" variant="flat">
          <div class="barometric-hero__content">
            ...
            <div class="barometric-hero__actions">
              <v-btn class="keyboard-toggle" color="primary" variant="outlined" :prepend-icon="keyboardMode === 'numeric' ? 'mdi-numeric' : 'mdi-function-variant'" @click="toggleKeyboardMode">
        ...
        <v-row>
          <v-col cols="12" md="6">
            <BarometricPointCard
              ...
              :copy-label="t('barometric.copyPointB')"
              ...
              @copy-point="copyPoint('B', 'A')"
            />
          </v-col>
          <v-col cols="12" md="6">
            <BarometricPointCard
              :title="t('barometric.pointB')"
              :copy-label="t('barometric.copyPointA')"
...
const { state, points, updateInput, updateUnit, copyPoint, commitInput } = useBarometricConverter();
...
function buildChartOption(): EChartsOption {
  const pointHeights = [points.value.A.heightMeters, points.value.B.heightMeters].filter((value): value is number => value !== null && Number.isFinite(value));
  ...
  const minimumAltitude = Math.min(defaultMinimumAltitude, minimumPointAltitude < defaultMinimumAltitude ? minimumPointAltitude - rangePaddingMeters : defaultMinimumAltitude);
  const maximumAltitude = Math.max(defaultMaximumAltitude, maximumPointAltitude > defaultMaximumAltitude ? maximumPointAltitude + rangePaddingMeters : defaultMaximumAltitude);
  ...
  const chartGrid = compactChart.value ? { left: 8, right: 4, top: 16, bottom: 44, containLabel: true } : { left: 36, right: 8, top: 20, bottom: 48, containLabel: true };

  const pointSeries = [
    {
      name: "A",
      value:
        points.value.A.heightMeters !== null && points.value.A.pressurePa !== null
    ...
    },
    {
      name: "B",
      value:
        points.value.B.heightMeters !== null && points.value.B.pressurePa !== null
...
watch(
  () => [points.value.A.heightMeters, points.value.A.pressurePa, points.value.B.heightMeters, points.value.B.pressurePa, chartHeightUnit.value, chartPressureUnit.value, locale.value],
  updateChart,
);
watch(() => route.fullPath, generateQRCode);

const heightUnitItems = computed(() =>
  HEIGHT_UNITS.map((value) => ({
    value,
    title: t(`barometric.units.height.${value}`),
  })),
);

const pressureUnitItems = computed(() =>
  PRESSURE_UNITS.map((value) => ({
    value,
