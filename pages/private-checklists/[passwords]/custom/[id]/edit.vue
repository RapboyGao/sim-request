...
import { privateChecklistsHomeRoute, privateCustomChecklistRoute } from '~/utils/checklist-routes'
import { builtinChecklists } from '~/data/checklists'
definePageMeta({ layout: 'checklists' })
const route = useRoute(); const router = useRouter(); const passwords = computed(() => String(route.params.passwords || ''))
const { customChecklists, updateChecklist } = useChecklists({ scope: 'private', builtins: computed(() => [...builtinChecklists]) })
const checklist = computed(() => customChecklists.value.find((item) => item.id === String(route.params.id || '')))
function goHome() { router.push(privateChecklistsHomeRoute(passwords.value)) }
function cancel() { checklist.value ? router.push(privateCustomChecklistRoute(passwords.value, checklist.value.id)) : goHome() }
function save(next: Checklist) { if (next.source === 'custom') { updateChecklist(next); router.push(privateCustomChecklistRoute(passwords.value, next.id)) } }
